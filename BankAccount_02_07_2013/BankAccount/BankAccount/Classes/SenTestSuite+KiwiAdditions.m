...
+ (void)patchTestSuiteForTestCaseClassIMP {
    ...
    IMP focusedSuite = imp_implementationWithBlock(^(id _self, Class aClass){
        return ([[KWExampleGroupBuilder sharedExampleGroupBuilder] isFocused] && ![_self testSuiteClassHasFocus:aClass]) ? nil : (void *)CFBridgingRetain([_self performSelector:@selector(__testSuiteForTestCaseClass:) withObject:aClass]);
    });
